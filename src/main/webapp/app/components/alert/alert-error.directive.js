(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('vivaxDataManagerApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-vivaxDataManagerApp-error');
                var entityKey = httpResponse.headers('X-vivaxDataManagerApp-params');
                if (errorHeader) {
